...
journey
    ...
    section Добавление в корзину
      ...
      Система проверяет наличие товара на складе: 5: Система
      Система обновляет содержимое корзины: 5: Система
      Пользователь проверяет корзину и редактирует список товаров: 4: Пользователь
    section Оформление заказа
      ...
      Пользователь получает уведомление о принятии заказа: 5: Пользователь
    section Управление доставкой
      Пользователь проверяет статус выполнения заказа: 4: Пользователь
      Система обновляет статус заказа (обработка, выполнен): 5: Система
      Пользователь получает доступ к купленной валюте (код/перевод внутриигровой валюты): 5: Пользователь
    section Завершение
      Пользователь оставляет отзыв о процессе покупки: 4: Пользователь
      Система сохраняет отзыв и обновляет рейтинг магазина: 5: Система
